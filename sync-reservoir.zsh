#!/usr/bin/env zsh
# Hydrate or update every Lean Reservoir package in reservoir.tsv.
# Mirrors the sparse-checkout convention of the main corpus:
#   keep *.lean, lakefile.*, lean-toolchain, lake-manifest.json, README*
# Logs clone failures to reservoir-missing.now for inspection.
set -u
ROOT="$(cd "$(dirname "$0")" && pwd)"
cd "$ROOT" || exit 1
FAIL_LOG="reservoir-missing.now"
: > "$FAIL_LOG"
COUNT=0
FAILED=0
