...
| README/project documentation | Normally auxiliary only | Useful for terminology/source discovery, but not a formal theorem/definition result. Do not physically discard merely to clean primary search. |
...
README content is ambiguous in value, not in role.  It is not itself formal
mathematical content, so it should not normally occupy primary theorem-search
slots.  It can still improve source discovery by naming concepts, describing
module layout, or pointing to the relevant implementation.

The preferred architecture is therefore a separate documentation/source
...
The 2026-09-14 audit found over a thousand README files physically present in
the Zoekt index.  Their presence is index/storage bloat even when the browser's
formal-extension filter prevents them from appearing in ordinary results.
